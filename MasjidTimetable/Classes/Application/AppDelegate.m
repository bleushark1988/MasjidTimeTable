...
//                UNTimeIntervalNotificationTrigger *trigger = [UNTimeIntervalNotificationTrigger triggerWithTimeInterval:timeInterval repeats:NO];
            ...
            NSDateComponents *dateComponents = [calendar components: NSCalendarUnitHour | NSCalendarUnitMinute fromDate:fireDate];
            dateComponents.timeZone = [NSTimeZone systemTimeZone];
            
            UNCalendarNotificationTrigger *trigger = [UNCalendarNotificationTrigger triggerWithDateMatchingComponents:dateComponents repeats:YES];
            
            NSString *str_identifier = [NSString stringWithFormat:@"%@ %@",event.title,event.time];
            
            UNNotificationRequest *request = [UNNotificationRequest requestWithIdentifier:str_identifier content:content trigger:trigger];
...
//                                                      triggerWithTimeInterval:timeInterval repeats:NO];
        ...
        NSDateComponents *dateComponents = [calendar components: NSCalendarUnitHour | NSCalendarUnitMinute fromDate:jamaatDate];
        dateComponents.timeZone = [NSTimeZone systemTimeZone];
        
        UNCalendarNotificationTrigger *trigger = [UNCalendarNotificationTrigger triggerWithDateMatchingComponents:dateComponents repeats:YES];
        
        NSString *str_identifier = [NSString stringWithFormat:@"%@ %@",info[1],info[2]];
...
- (NSArray*)getFireDateInfoWithIdentifier:(NSString *)identifier dayIndex:(int)dayIndex
{
  ...
  if ([identifier hasSuffix:@"B"]) {
   ...
   if ([identifier isEqualToString:@"zoharB"] || [identifier isEqualToString:@"zoharJ"]) {
       //fajarTime = @"11.02";
       //isTimeFromatAM = YES;
       
       //Jigar testing
       if ([fajarTime integerValue] == 12  || [fajarTime integerValue] < 6) {
        fajarTime =  [Utils reformatTimeTo24timeFormat:fajarTime isAMformat:NO withTimeFormat:tableTimeFormat.format];
       }else{
...
{// TODO: notification upgrade
    ...
    if (beginnnerSoundSettings.soundName) {
        ...
        if (notificationExist) {
            UILocalNotification *notification = [[UILocalNotification alloc] init];
            notification.category = @"Alert";
            notification.alertBody = [NSString stringWithFormat:@"%@ begins in %@ mins at %@ ", info[1], fireDateInfo[1], fireDateInfo[2]];
...
{
  NSString *str_currentTimeFloat = [[NSString stringWithFormat:@"%f",[Utils getFromTimeFloatValue:[Utils getCurrentTime]]] substringToIndex:3];
    
  if ([str_currentTimeFloat isEqualToString:@"0.0"]) {
...
#pragma mark - UserNotificationCenter delegate method
...
- (void)sampleDemoApp:(NSArray*)info {
    ...
    content.title = [NSString localizedUserNotificationStringForKey:@"Salah Reminder" arguments:nil];
    content.categoryIdentifier = @"Alert";
    
    if ([identifier hasSuffix:@"B"]) {
        content.body = [NSString stringWithFormat:@"%@ begins in %@ mins at %@ ", info[1], fireDateInfo[1], fireDateInfo[2]];
        NSString *soundName = [self isTimetableSoundOffWithIdentifier:identifier] ? @"muteSound.wav" : beginnnerSoundSettings.soundName;
        if ([soundName isEqualToString:@"default"]) {
            content.sound = [UNNotificationSound defaultSound];
        } else {
